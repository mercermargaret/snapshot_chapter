...
for (k in 1:length(pred_list)) {
  ...
  for (l in 1:length(prey_list)) {
    ...
    observations_low <- filter(obs_within_range, Humans_Per_Camera_Per_Day == 0)
    site_info_low <- filter(deps_within_range, Humans_Per_Camera_Per_Day == 0)
    ...
    result <- tryCatch(
      {
        # Attempt to fit your model
        prey_model_low <- occu(~ days_scaled + pred_occupancy ~ Predator_Occupancy + (1 | Array_Year), data = umf)
        # Return the model if successful
        list(success = TRUE, model = prey_model_low)
        summary_prey_low <- summary(prey_model_low)
      },
      error = function(e) {
        # If an error occurs, do something else
        message("An error occurred: ", e$message)
        effect_pred_on_prey_low <- NA
        p_value_effect_pred_on_prey_low <- NA
        lower_CI_low <- NA
        upper_CI_low <- NA
    ...
    # } else {
    #   print(result$alternative_action)
    # }
    # 
    ...
    observations_high <- filter(obs_within_range, Humans_Per_Camera_Per_Day > 0)
    site_info_high <- filter(deps_within_range, Humans_Per_Camera_Per_Day > 0)
...
write_csv(results_occ, "results/pred_prey_occupancy_with_detection_results_3.csv")
...
# write results as csv
write_csv(results_det, "results/pred_prey_detection_results_3.csv")
