...
for (i in 1:length(pred_list)) {
  ...
  for (j in 1:length(prey_list)) {
    ...
    low_dist <- filter(pair, Humans_Per_Camera_Per_Day == 0)
    ...
    if (length(low_pred) < 5 | length(low_prey) < 5) {
      overlap_low <- NA
      bootstrap_low <- NA
    } else {
    ...
    high_dist <- filter(pair, Humans_Per_Camera_Per_Day > 0)
    ...
    if (length(high_pred) < 5 | length(high_prey) < 5) {
      overlap_high <- NA
      bootstrap_high <- NA
    } else {
    ...
    if (length(high_pred) < 5 | length(high_prey) < 5 | length(low_pred) < 5 | length(low_prey) < 5) {
      t_test <- data.frame(c(0))
      t_test$p.value <- NA
    } else {
    ...
    results[k, 7] <- length(low_pred)
    results[k, 8] <- length(high_pred)
    results[k, 9] <- length(low_prey)
    results[k, 10] <- length(high_prey)
...
write.csv(results, "results/pred_prey_overlap_results_3.csv")
