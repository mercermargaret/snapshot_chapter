...
# (pull out value to use as covariate for high prey occupancy)
# margaret mercer
# august 5, 2024
...
for(i in 1:length(unique(observations$Site_Name))){
  ...
  data_i <- observations[which(observations$Site_Name == site_i & 
                                 observations$Species_Name == "Puma concolor"), ]
...
# species <- subset(observations, Species_Name == "Puma concolor")
...
write.csv(hist, "data/hist_high_pred.csv")
write.csv(DOY, "data/DOY_high_pred.csv")
write.csv(survey_days, "data/survey_days_high_pred.csv")
write.csv(site_info, "data/site_info_high_pred.csv")
...
hist <- read.csv("data/hist_high_pred.csv")
DOY <- read.csv("data/DOY_high_pred.csv")
survey_days <- read.csv("data/survey_days_high_pred.csv")
site_info <- read.csv("data/site_info_high_pred.csv")
...
pred_model <- occu(~ days_scaled ~ 1 + (1 | Array), data = umf) 
# what do here? which value do we pull out, if we're accounting for days scaled? How do we deal with random effects?

summary(pred_model)

# and now pull out finite occupancy estimates for use in prey analysis ####
...
site_info <- as.data.frame(select(site_info, -X))
...
# # Convert list column
# site_info$list_col <- sapply(site_info$list_col, function(x) paste(unlist(x), collapse = ";"))
...
write_csv(site_info, "data/site_info_high_prey.csv")
