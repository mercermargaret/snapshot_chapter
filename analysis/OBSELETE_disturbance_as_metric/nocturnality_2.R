# analysis of nocturnality for all species using disturance as a metric
# Margaret Mercer
# sept 27, 2024
...
for (i in 1:length(species_list)) {
    ...
    # now add disturbance as column
    unique_species <- species %>% distinct(Site_Name, .keep_all = TRUE)
    unique_species <- unique_species %>% dplyr::select(Site_Name, Disturbance)
    props_updated <- props %>% 
    ...
    low <- filter(props_updated, Disturbance < median(props_updated$Disturbance, na.rm = TRUE))
    high <- filter(props_updated, Disturbance >= median(props_updated$Disturbance, na.rm = TRUE))
