# Black Bear and Moose Overlap Analysis
# Margaret Mercer
# April 27, 2024
...
pred_range <- st_read('data/subset_shape_files/Black Bear')
...
# Black bear: slight morning/evening peak with low human presence; pretty consistently active with higher human presence, with maybe a slight shift toward nocturnality
# Moose: low presence has clear peaks at 7am and 7pm; goes crazy with high human presence, with peak at 11pm, 10am, and 4pm
# Overlap t test: higher overlap with higher human presence


# Print results to "results" folder ####

# overlap graph
pdf("results/overlap_graphs/blackbear_moose_low.pdf")
overlapPlot(low_pred, low_prey)
legend('topright', c("Black Bear", "Moose"), lty=c(1,2), col=c(1,4), bty='n')
dev.off()

pdf("results/overlap_graphs/blackbear_moose_high.pdf")
overlapPlot(high_pred, high_prey)
legend('topright', c("Black Bear", "Moose"), lty=c(1,2), col=c(1,4), bty='n')
dev.off()
...
# t = 32.676, df = 398, p-value < 2.2e-16
...
# cutoff at median
# data:  bootstrap_low and bootstrap_high
# t = 23.277, df = 398, p-value < 2.2e-16
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#   0.1033600 0.1224302
# sample estimates:
