# Grizzly and Human Overlap Analysis
# Margaret Mercer
# May 9, 2024
...
low_animal <- time_radians[low_dist$Species_Name == 'Ursus arctos']
low_human <- time_radians[low_dist$Species_Name == "Homo sapiens" | low_dist$Species_Name == "Vehicle"]

pdf("results/overlap_graphs/grizzly_human_low.pdf")
overlapPlot(low_animal, low_human)
legend('topright', c("Grizzly", "Human"), lty=c(1,2), col=c(1,4), bty='n')
dev.off()
...
high_animal <- time_radians[high_dist$Species_Name == 'Ursus arctos']
high_human <- time_radians[high_dist$Species_Name == "Homo sapiens" | high_dist$Species_Name == "Vehicle"]

pdf("results/overlap_graphs/grizzly_human_high.pdf")
...
# Grizzly: mostly noct; strong crepuscular peaks with lower human activity; higher noct with higher human act but also slightly more diurnal with higher human act
# Human: mostly diurnal; less spread with higher human presence
# Overlap: increases with increasing human presence

# t test results
# data:  bootstrap_low and bootstrap_high
# t = -47.119, df = 398, p-value < 2.2e-16
