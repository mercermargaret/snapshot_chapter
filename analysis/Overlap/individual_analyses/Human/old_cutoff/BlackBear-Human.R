# Black Bear and Human Overlap Analysis
# Margaret Mercer
# May 9, 2024
...
low_animal <- time_radians[low_dist$Species_Name == 'Ursus americanus']
low_human <- time_radians[low_dist$Species_Name == "Homo sapiens" | low_dist$Species_Name == "Vehicle"]

pdf("results/overlap_graphs/blackbear_human_low.pdf")
overlapPlot(low_animal, low_human)
legend('topright', c("Black Bear", "Human"), lty=c(1,2), col=c(1,4), bty='n')
dev.off()
...
high_animal <- time_radians[high_dist$Species_Name == 'Ursus americanus']
high_human <- time_radians[high_dist$Species_Name == "Homo sapiens" | high_dist$Species_Name == "Vehicle"]

pdf("results/overlap_graphs/blackbear_human_high.pdf")
...
# Black bear: maybe sliiighlty more nocturnality with higher human presence
# Human: wider spread with higher human activity
# Overlap: no significant change

# t test results
# data:  bootstrap_low and bootstrap_high
# t = -0.30543, df = 398, p-value = 0.7602
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#   -0.002434356  0.001779669
# sample estimates:
#   mean of x mean of y 
# 0.4927097 0.4930370 