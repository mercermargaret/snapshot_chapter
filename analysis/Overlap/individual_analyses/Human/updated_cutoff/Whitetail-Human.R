# Whitetail and Human Overlap Analysis
# Margaret Mercer
# May 9, 2024
...
# get range data
range <- st_read('data/subset_shape_files/Whitetail')
...
# Overlap Analysis ####
species <- filter(df_inside, Species_Name == 'Odocoileus virginianus')
...
low_animal <- time_radians[low_dist$Species_Name == 'Odocoileus virginianus']
low_human <- time_radians[low_dist$Species_Name == "Homo sapiens" | low_dist$Species_Name == "Vehicle"]

pdf("results/overlap_graphs/whitetail_human_low.pdf")
overlapPlot(low_animal, low_human)
legend('topright', c("Whitetail", "Human"), lty=c(1,2), col=c(1,4), bty='n')
dev.off()
...
high_animal <- time_radians[high_dist$Species_Name == 'Odocoileus virginianus']
high_human <- time_radians[high_dist$Species_Name == "Homo sapiens" | high_dist$Species_Name == "Vehicle"]

pdf("results/overlap_graphs/whitetail_human_high.pdf")
...
# Overlap: increasing with increasing human presence

# t test results
# data:  bootstrap_low and bootstrap_high
# t = -224.57, df = 398, p-value < 2.2e-16
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#   -0.07638445 -0.07505870
# sample estimates:
#   mean of x mean of y 
# 0.3983528 0.4740744 

# t test results species only median
# data:  bootstrap_low and bootstrap_high
# t = -230.99, df = 398, p-value < 2.2e-16
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#   -0.08127807 -0.07990625
# sample estimates:
#   mean of x mean of y 
# 0.4002588 0.4808510
