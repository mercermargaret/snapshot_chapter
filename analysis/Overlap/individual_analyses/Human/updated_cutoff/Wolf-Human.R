# Wolf and Human Overlap Analysis
# Margaret Mercer
# May 9, 2024
...
# get range data
range <- st_read('data/subset_shape_files/Wolf')
...
# Overlap Analysis ####
species <- filter(df_inside, Species_Name == 'Canis lupus')
...
low_animal <- time_radians[low_dist$Species_Name == 'Canis lupus']
low_human <- time_radians[low_dist$Species_Name == "Homo sapiens" | low_dist$Species_Name == "Vehicle"]

pdf("results/overlap_graphs/wolf_human_low.pdf")
overlapPlot(low_animal, low_human)
legend('topright', c("Wolf", "Human"), lty=c(1,2), col=c(1,4), bty='n')
dev.off()
...
high_animal <- time_radians[high_dist$Species_Name == 'Canis lupus']
high_human <- time_radians[high_dist$Species_Name == "Homo sapiens" | high_dist$Species_Name == "Vehicle"]

pdf("results/overlap_graphs/Wolf_human_high.pdf")
...
# Wolf: strong morning peak with high human presence
...
# t = -35.781, df = 398, p-value < 2.2e-16
...
# data:  bootstrap_low and bootstrap_high
# t = -19.448, df = 398, p-value < 2.2e-16
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#   -0.07607497 -0.06210643
# sample estimates:
#   mean of x mean of y 
# 0.3296173 0.3987080 
