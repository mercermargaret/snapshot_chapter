# Grizzly and Elk Overlap Analysis
# Margaret Mercer
# April 16, 2024
...
# sf_use_s2(FALSE) 
# for some reason it's not working for this particular analysis to switch the spherical geometry 
    # on and then back off, so we'll just leave it as is
...
# Grizzly: clear shift toward noct with high human presence
# Elk: clear shift toward diurnality with high human presence
# Overlap: MUCH higher overlap in low human presence than high

# Print results to "results" folder ####

# overlap graph
pdf("results/overlap_graphs/grizzly_elk_low.pdf")
overlapPlot(low_pred, low_prey)
legend('topright', c("Grizzly", "Elk"), lty=c(1,2), col=c(1,4), bty='n')
dev.off()

pdf("results/overlap_graphs/grizzly_elk_high.pdf")
overlapPlot(high_pred, high_prey)
legend('topright', c("Grizzly", "Elk"), lty=c(1,2), col=c(1,4), bty='n')
dev.off()
...
# t = 91.024, df = 398, p-value < 2.2e-16
...
# cutoff at median
# data:  bootstrap_low and bootstrap_high
# t = 87.116, df = 398, p-value < 2.2e-16
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#   0.3428832 0.3587162
# sample estimates:
