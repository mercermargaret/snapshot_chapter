# Grizzly and Moose Overlap Analysis
# Margaret Mercer
# April 16, 2024
...
# Grizzly: clear shift toward nocturnality with higher human presence
# Moose: become more diurnal with higher human presence
# Overlap: much higher overlap with low human presence than with high

# Print results to "results" folder ####

# overlap graph
pdf("results/overlap_graphs/grizzly_moose_low.pdf")
overlapPlot(low_pred, low_prey)
legend('topright', c("Grizzly", "Moose"), lty=c(1,2), col=c(1,4), bty='n')
dev.off()

pdf("results/overlap_graphs/grizzly_moose_high.pdf")
overlapPlot(high_pred, high_prey)
legend('topright', c("Grizzly", "Moose"), lty=c(1,2), col=c(1,4), bty='n')
dev.off()
...
# t = 37.041, df = 398, p-value < 2.2e-16
...
# data:  bootstrap_low and bootstrap_high
# t = 51.009, df = 398, p-value < 2.2e-16
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#   0.3230585 0.3489589
# sample estimates:
