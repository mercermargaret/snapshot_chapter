# Black Bear and Mule Deer Overlap Analysis
# Margaret Mercer
# April 27, 2024
...
# Black bear: 6pm peak higher with higher human presence and rest of day lower
# Mule Deer: peaks much flatter with high human presence; 9am and 6am peaks still present (although morning peak shifts slightly earlier with higher presence)
# Overlap t test: higher overlap with lower human presence


# Print results to "results" folder ####

# overlap graph
pdf("results/overlap_graphs/blackbear_muledeer_low.pdf")
overlapPlot(low_pred, low_prey)
legend('topright', c("Black Bear", "Mule Deer"), lty=c(1,2), col=c(1,4), bty='n')
dev.off()

pdf("results/overlap_graphs/blackbear_muledeer_high.pdf")
overlapPlot(high_pred, high_prey)
legend('topright', c("Black Bear", "Mule Deer"), lty=c(1,2), col=c(1,4), bty='n')
dev.off()
