# Coyote and Cottontail Overlap Analysis
# Margaret Mercer
# May 1, 2024
...
# get range data
prey_range <- st_read('data/subset_shape_files/Cottontail')
...
low_prey <- time_radians[low_dist$Species_Name == 'Sylvilagus aquaticus' |
                           low_dist$Species_Name == 'Sylvilagus audubonii'|
                           low_dist$Species_Name == 'Sylvilagus bachmani' |
                           low_dist$Species_Name == 'Sylvilagus floridanus' |
                           low_dist$Species_Name == 'Sylvilagus nuttallii' |
                           low_dist$Species_Name == 'Sylvilagus obscurus' |
                           low_dist$Species_Name == 'Sylvilagus palustris']
...
high_prey <- time_radians[high_dist$Species_Name == 'Sylvilagus aquaticus' |
                          high_dist$Species_Name == 'Sylvilagus audubonii'|
                          high_dist$Species_Name == 'Sylvilagus bachmani' |
                          high_dist$Species_Name == 'Sylvilagus floridanus' |
                          high_dist$Species_Name == 'Sylvilagus nuttallii' |
                          high_dist$Species_Name == 'Sylvilagus obscurus' |
                          high_dist$Species_Name == 'Sylvilagus palustris']
...
# Coyote: not much difference; tiiiiiny shift toward nocturnality with higher human presence
# Cottontail: not much of a trend at all; maybe just a little bit of a higher peak at 6am with higher human presence
# Overlap: not much overlap difference


# Print results to "results" folder ####

# overlap graph
pdf("results/overlap_graphs/coyote_cottontail_low.pdf")
overlapPlot(low_pred, low_prey)
legend('topright', c("Coyote", "Cottontail"), lty=c(1,2), col=c(1,4), bty='n')
dev.off()

pdf("results/overlap_graphs/coyote_cottontail_high.pdf")
overlapPlot(high_pred, high_prey)
legend('topright', c("Coyote", "Cottontail"), lty=c(1,2), col=c(1,4), bty='n')
dev.off()
...
# t = 1.2571, df = 398, p-value = 0.2095
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#   -0.0004857819  0.0022087904
# sample estimates:
#   mean of x mean of y 
# 0.8748539 0.8739924 
...
# cutoff at median
# data:  bootstrap_low and bootstrap_high
# t = 0.96603, df = 398, p-value = 0.3346
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#   -0.0006427288  0.0018846231
# sample estimates:
#   mean of x mean of y 
# 0.8752354 0.8746144 
