# Puma and Moose Overlap Analysis
# Margaret Mercer
# April 16, 2024
...
pair <- filter(df_inside, Species_Name == 'Puma concolor' | Species_Name == 'Alces alces') 
...
# Moose have some weird lumps throughout but it seems they shift to more diurnal with higher human activity
# Greater overlap between puma and moose in areas of LOW human presence than in areas of HIGH human presence (p value = 0.07)


# Print results to "results" folder ####

# overlap graph
pdf("results/overlap_graphs/puma_moose_low.pdf")
overlapPlot(low_pred, low_prey)
legend('topright', c("Puma", "Moose"), lty=c(1,2), col=c(1,4), bty='n')
dev.off()

pdf("results/overlap_graphs/puma_moose_high.pdf")
overlapPlot(high_pred, high_prey)
legend('topright', c("Puma", "Moose"), lty=c(1,2), col=c(1,4), bty='n')
dev.off()
...
# first t test (before changing spatial stuff)
# data:  bootstrap_low and bootstrap_high
# t = 39.225, df = 398, p-value < 2.2e-16
...
# second t test
# data:  bootstrap_low and bootstrap_high
# t = 40.901, df = 398, p-value < 2.2e-16
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#   0.3511333 0.3865927
# sample estimates:
...
# data:  bootstrap_low and bootstrap_high
# t = 32.497, df = 398, p-value < 2.2e-16
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#   0.2297277 0.2593131
# sample estimates:
...
# data:  bootstrap_low and bootstrap_high
# t = 3.4589, df = 398, p-value = 0.0006009
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#   0.01080126 0.03924731
# sample estimates:
#   mean of x mean of y 
# 0.6744750 0.6494508
