# Snowshoe Hare and Human Overlap Analysis
# Margaret Mercer
# May 9, 2024
...
low_animal <- time_radians[low_dist$Species_Name == 'Lepus americanus']
low_human <- time_radians[low_dist$Species_Name == "Homo sapiens" | low_dist$Species_Name == "Vehicle"]

pdf("results/overlap_graphs/snowshoe_human_low.pdf")
overlapPlot(low_animal, low_human)
legend('topright', c("Snowshoe Hare", "Human"), lty=c(1,2), col=c(1,4), bty='n')
dev.off()
...
high_animal <- time_radians[high_dist$Species_Name == 'Lepus americanus']
high_human <- time_radians[high_dist$Species_Name == "Homo sapiens" | high_dist$Species_Name == "Vehicle"]

pdf("results/overlap_graphs/snowshoe_human_high.pdf")
...
# Snowshoe Hare: no change (exclusively nocturnal)
# Human: wider spread with higher human activity
# Overlap: slight increase

# t test results
# data:  bootstrap_low and bootstrap_high
# t = -18.607, df = 398, p-value < 2.2e-16
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#   -0.01590330 -0.01286392
# sample estimates:
#   mean of x mean of y 
# 0.1052136 0.1195972 