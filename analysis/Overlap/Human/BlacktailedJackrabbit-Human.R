# Blacktailed Jackrabbit and Human Overlap Analysis
# Margaret Mercer
# May 9, 2024
...
low_animal <- time_radians[low_dist$Species_Name == 'Lepus californicus']
low_human <- time_radians[low_dist$Species_Name == "Homo sapiens" | low_dist$Species_Name == "Vehicle"]

pdf("results/overlap_graphs/blacktailedjackrabbit_human_low.pdf")
overlapPlot(low_animal, low_human)
legend('topright', c("Blacktailed Jackrabbit", "Human"), lty=c(1,2), col=c(1,4), bty='n')
dev.off()
...
high_animal <- time_radians[high_dist$Species_Name == 'Lepus californicus']
high_human <- time_radians[high_dist$Species_Name == "Homo sapiens" | high_dist$Species_Name == "Vehicle"]

pdf("results/overlap_graphs/blacktailedjackrabbit_human_high.pdf")
...
# Blacktailed Jackrabbit: not much change. Higher AM crepuscular peak with higher human presence
# Human: wider spread with higher human activity
# Overlap: increase

# t test results
# data:  bootstrap_low and bootstrap_high
# t = -83.072, df = 398, p-value < 2.2e-16
