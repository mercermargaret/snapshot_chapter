# Moose and Human Overlap Analysis
# Margaret Mercer
# May 9, 2024
...
# get range data
range <- st_read('data/subset_shape_files/Moose')
...
# Overlap Analysis ####
species <- filter(df_inside, Species_Name == 'Alces alces')
...
low_animal <- time_radians[low_dist$Species_Name == 'Alces alces']
low_human <- time_radians[low_dist$Species_Name == "Homo sapiens" | low_dist$Species_Name == "Vehicle"]

pdf("results/overlap_graphs/moose_human_low.pdf")
overlapPlot(low_animal, low_human)
legend('topright', c("Moose", "Human"), lty=c(1,2), col=c(1,4), bty='n')
dev.off()
...
high_animal <- time_radians[high_dist$Species_Name == 'Alces alces']
high_human <- time_radians[high_dist$Species_Name == "Homo sapiens" | high_dist$Species_Name == "Vehicle"]

pdf("results/overlap_graphs/moose_human_high.pdf")
...
# Moose: more diurnal with higher human activity
# Human: wider spread with higher human activity
# Overlap: much higher overlap with higher human activity

# t test results
# data:  bootstrap_low and bootstrap_high
# t = -102.39, df = 398, p-value < 2.2e-16
...
# t test results cutoff at median
# data:  bootstrap_low and bootstrap_high
# t = -43.267, df = 398, p-value < 2.2e-16
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#   -0.1437116 -0.1312194
# sample estimates:
