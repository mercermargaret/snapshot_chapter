# Skunk and Human Overlap Analysis
# Margaret Mercer
# May 23, 2024
...
# get range data
range <- st_read('data/subset_shape_files/Striped Skunk')
...
# Overlap Analysis ####
species <- filter(df_inside, Species_Name == 'Mephitis mephitis')
...
low_animal <- time_radians[low_dist$Species_Name == 'Mephitis mephitis']
low_human <- time_radians[low_dist$Species_Name == "Homo sapiens" | low_dist$Species_Name == "Vehicle"]

pdf("results/overlap_graphs/skunk_human_low.pdf")
overlapPlot(low_animal, low_human)
legend('topright', c("Skunk", "Human"), lty=c(1,2), col=c(1,4), bty='n')
dev.off()
...
high_animal <- time_radians[high_dist$Species_Name == 'Mephitis mephitis']
high_human <- time_radians[high_dist$Species_Name == "Homo sapiens" | high_dist$Species_Name == "Vehicle"]

pdf("results/overlap_graphs/skunk_human_high.pdf")
...
# t = -27.708, df = 398, p-value < 2.2e-16
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#   -0.02036125 -0.01766337
# sample estimates:
#   mean of x  mean of y 
# 0.09196973 0.11098203 

# t test results with cutoff at median of species
# data:  bootstrap_low and bootstrap_high
# t = -30.276, df = 398, p-value < 2.2e-16
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#   -0.02160065 -0.01896645
# sample estimates:
#   mean of x  mean of y 
# 0.09309292 0.11337647
