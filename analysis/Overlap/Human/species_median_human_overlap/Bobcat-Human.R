# Bobcat and Human Overlap Analysis
# Margaret Mercer
# May 22, 2024
...
# get range data
range <- st_read('data/subset_shape_files/Bobcat')
...
# Overlap Analysis ####
species <- filter(df_inside, Species_Name == 'Lynx rufus')
...
low_animal <- time_radians[low_dist$Species_Name == 'Lynx rufus']
low_human <- time_radians[low_dist$Species_Name == "Homo sapiens" | low_dist$Species_Name == "Vehicle"]

pdf("results/overlap_graphs/bobcat_human_low.pdf")
overlapPlot(low_animal, low_human)
legend('topright', c("Bobcat", "Human"), lty=c(1,2), col=c(1,4), bty='n')
dev.off()
...
high_animal <- time_radians[high_dist$Species_Name == 'Lynx rufus']
high_human <- time_radians[high_dist$Species_Name == "Homo sapiens" | high_dist$Species_Name == "Vehicle"]

pdf("results/overlap_graphs/bobcat_human_high.pdf")
...
# Bobcat: more noct as human presesnce increases
# Human: wider spread
# Overlap: increases

# t test results
# data:  bootstrap_low and bootstrap_high
# t = -27.836, df = 398, p-value < 2.2e-16
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#   -0.03849351 -0.03341491
# sample estimates:
#   mean of x mean of y 
# 0.3931829 0.4291371 
...
# t test results 
# data:  bootstrap_low and bootstrap_high
# t = -22.807, df = 398, p-value < 2.2e-16
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#   -0.03069985 -0.02582722
# sample estimates:
#   mean of x mean of y 
# 0.3873677 0.4156312 