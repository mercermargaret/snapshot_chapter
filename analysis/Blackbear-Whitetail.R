# Black Bear and Whitetail Overlap Analysis
# Margaret Mercer
# April 16, 2024
...
# unifying the black bear range doesn't work (loop __ not valid; edge __ crosses edge __)

prey_range_unified <- st_make_valid(st_union(prey_range))
# this works but doesn't fix the problem

sf_use_s2(FALSE) # turn off spherical geometry and then run it!

# now calculate overlap
range_overlap <- st_make_valid(st_intersection(prey_range_unified, pred_range))
# so this part is actually working now (the map worked, etc)
...
# sf_use_s2(FALSE)
# if I leave this on, the following code breaks ("Error in wk_handle.wk_wkb(wkb, s2_geography_writer(oriented = oriented, 
# Loop __ is not valid: Edge __ crosses edge __")
# if I switch it off, it gets stuck and just thinks for forever :')

# Perform the point-in-polygon test
inside <- st_within(points_sf, range_overlap, sparse = FALSE, dimension = "polygon") # AND IT DOESN"T WORK ASDKGHLKH
