# Whitetailed Jackrabbit and Human Overlap Analysis
# Margaret Mercer
# May 9, 2024
...
low_animal <- time_radians[low_dist$Species_Name == 'Lepus townsendii']
low_human <- time_radians[low_dist$Species_Name == "Homo sapiens" | low_dist$Species_Name == "Vehicle"]

pdf("results/overlap_graphs/whitetailedjackrabbit_human_low.pdf")
overlapPlot(low_animal, low_human)
legend('topright', c("Whitetailed Jackrabbit", "Human"), lty=c(1,2), col=c(1,4), bty='n')
dev.off()
...
high_animal <- time_radians[high_dist$Species_Name == 'Lepus townsendii']
high_human <- time_radians[high_dist$Species_Name == "Homo sapiens" | high_dist$Species_Name == "Vehicle"]

pdf("results/overlap_graphs/whitetailedjackrabbit_human_high.pdf")
...
# Whitetailed Jackrabbit: sliiiight shift further into day with higher human activity
# Human: wider spread with higher human activity
# Overlap: increases with higher human activity

# t test results
# data:  bootstrap_low and bootstrap_high
# t = -65.407, df = 398, p-value < 2.2e-16
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#   -0.10108445 -0.09518514
# sample estimates:
#   mean of x  mean of y 
# 0.08003454 0.17816934 