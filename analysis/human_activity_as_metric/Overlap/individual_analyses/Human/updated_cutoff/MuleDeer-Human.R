# Mule Deer and Human Overlap Analysis
# Margaret Mercer
# May 9, 2024
...
# get range data
range <- st_read('data/subset_shape_files/Mule Deer')
...
# Overlap Analysis ####
species <- filter(df_inside, Species_Name == 'Odocoileus hemionus')
...
low_animal <- time_radians[low_dist$Species_Name == 'Odocoileus hemionus']
low_human <- time_radians[low_dist$Species_Name == "Homo sapiens" | low_dist$Species_Name == "Vehicle"]

pdf("results/overlap_graphs/muledeer_human_low.pdf")
overlapPlot(low_animal, low_human)
legend('topright', c("Mule Deer", "Human"), lty=c(1,2), col=c(1,4), bty='n')
dev.off()
...
high_animal <- time_radians[high_dist$Species_Name == 'Odocoileus hemionus']
high_human <- time_radians[high_dist$Species_Name == "Homo sapiens" | high_dist$Species_Name == "Vehicle"]

pdf("results/overlap_graphs/muledeer_human_high.pdf")
...
# Mule deer: crepuscular; not much visible difference
# Human: wider spread with higher human activity
# Overlap: increasing overlap with higher human presence

# t test results
# data:  bootstrap_low and bootstrap_high
# t = -90.38, df = 398, p-value < 2.2e-16
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#   -0.05450910 -0.05218821
# sample estimates:
#   mean of x mean of y 
# 0.4212253 0.4745739 
...
# data:  bootstrap_low and bootstrap_high
# t = -68.358, df = 398, p-value < 2.2e-16
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#   -0.05456018 -0.05150966
# sample estimates:
#   mean of x mean of y 
# 0.4638472 0.5168822
