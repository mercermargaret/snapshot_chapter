...
# get range data
prey_range <- st_read('data/subset_shape_files/Elk')
...
pair <- filter(df_inside, Species_Name == 'Puma concolor' | Species_Name == 'Cervus canadensis') 
...
# Elk remain crepuscular throughout BUT dip in middle is SHALLOWER in high human areas! SO they're shifting to be more DIURNAL!
# Greater overlap between puma and elk in areas of LOW human presence than in areas of HIGH human presence (p value < 0.01)

# Print results to "results" folder ####

# overlap graph
pdf("results/overlap_graphs/puma_elk_low.pdf")
overlapPlot(low_pred, low_prey)
legend('topright', c("Puma", "Elk"), lty=c(1,2), col=c(1,4), bty='n')
dev.off()

pdf("results/overlap_graphs/puma_elk_high.pdf")
overlapPlot(high_pred, high_prey)
legend('topright', c("Puma", "Elk"), lty=c(1,2), col=c(1,4), bty='n')
dev.off()
...
# first t test (before standardizing spatial stuff)
# data:  bootstrap_low and bootstrap_high
# t = 42.954, df = 398, p-value < 2.2e-16
...
# second t test
# data:  bootstrap_low and bootstrap_high
# t = 41.386, df = 398, p-value < 2.2e-16
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#   0.1579855 0.1737433
# sample estimates:
...
# data:  bootstrap_low and bootstrap_high
# t = 15.91, df = 398, p-value < 2.2e-16
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#   0.05980862 0.07667290
# sample estimates:
#   mean of x mean of y 
# 0.7315977 0.6633569 
...
# data:  bootstrap_low and bootstrap_high
# t = 6.4189, df = 398, p-value = 3.909e-10
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#   0.01915635 0.03607103
# sample estimates:
#   mean of x mean of y 
# 0.7569538 0.7293401
