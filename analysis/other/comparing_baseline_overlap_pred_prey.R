...
for (j in 1:length(prey_list)) {
  ...
  for (i in 1:length(pred_list)) {
    ...
    time <- as.POSIXct(pair$Local_Time, format = "%H:%M:%S")
    ...
    pred <- time_radians[pair$Species_Name == pred_name]
    prey <- time_radians[pair$Species_Name == prey_name]

    overlap <- overlapEst(pred, prey, type="Dhat4")
    overlap
    
    # calculate bootstrap values
    bootstrap <- bootstrap(pred, prey, 200, smooth=TRUE, kmax=3, adjust=NA, n.grid=128, type="Dhat4", cores=1)
    # now calculate confidence intervals
    bootCI(overlap, bootstrap, conf = 0.95)
    ...
    results[k, 3] <- overlap
    results[k, 4] <- length(pred)
    results[k, 5] <- length(prey)
...
write.csv(results, "results/puma_vs_wolf_temporal_overlap.csv", row.names = FALSE)
