...
for (k in 1:length(pred_list)) {
  ...
  for (l in 1:length(prey_list)) {
    ...
    median <- median(deps_within_range$Disturbance, na.rm = TRUE)
    ...
    observations_low <- filter(obs_within_range, Disturbance < median)
    site_info_low <- filter(deps_within_range, Disturbance < median)
    ...
    observations_high <- filter(obs_within_range, Disturbance >= median)
    site_info_high <- filter(deps_within_range, Disturbance >= median)
...
write_csv(results_occ, "results/pred_prey_occupancy_with_detection_results_2.csv")
...
# write results as csv
write_csv(results_det, "results/pred_prey_detection_results_2.csv")
