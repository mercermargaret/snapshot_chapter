...
overlapPlot(low_species, high_species, linet = c(1,5), linec = c(NA, NA), linewidth = c(2, 2), olapcol = NA,
            rug=FALSE, main="Blank", ylab = NA, yaxt = "n")
# add line
abline(v=c(5.5, 18+47/60), lty=3, lwd = 3)


# just low
overlapPlot(low_species, high_species, linet = c(1,1), linec = c("red", NA), linewidth = c(3, 3), olapcol = NA,
            rug=FALSE, main="Low Human Activity", ylab = NA, yaxt = "n")
abline(v=c(5.5, 18+47/60), lty=3, lwd = 3)
# legend("bottomleft", c("Low Human Activity"), lty=c(1), lwd = c(2, 2), col=c("darkgreen"), bg="white")

# just High
overlapPlot(low_species, high_species, linet = c(1,1), linec = c(NA, "red"), linewidth = c(3, 3), olapcol = NA,
            rug=FALSE, main="High Human Activity", ylab = NA, yaxt = "n")
abline(v=c(5.5, 18+47/60), lty=3, lwd = 3)
# legend("bottomleft", c("High Human Activity"), lty=c(1), lwd = c(2, 2), col=c("darkgreen"), bg="white")


# both
overlapPlot(low_species, high_species, linet = c(1,5), linec = c("darkgreen", "purple"), linewidth = c(8, 8), olapcol = NA,
            rug=FALSE, main="Puma Activity in Wild Vs Urban Areas", ylab = NA, yaxt = "n")
abline(v=c(5.5, 18+47/60), lty=3, lwd = 3)
# legend("bottomleft", c("Low Human Activity", "High Human Activity"), lty=c(1,5), lwd = c(2, 2), col=c("darkgreen", "purple"), bg="white")
