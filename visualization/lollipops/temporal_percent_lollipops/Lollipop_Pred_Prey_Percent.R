...
lol <- ggplot(data, aes(x = Pairing, y = Percent_Change,
  ...
  geom_segment(aes(xend = Pairing, yend = 0), lwd = 2) +
  ...
  labs(x = "Mesocarnivore Prey     Ungulate Prey      Mesocarnivore Prey      Ungulate Prey", 
       y = "Percent Difference in Temporal Overlap with Higher Human Activity", 
  ...
  geom_text(aes(x = Pairing, y = min(Percent_Change) - 0.09, label = Prey_Common), hjust = 0, vjust = 0.5, color = "black", size = 10) 
lol
